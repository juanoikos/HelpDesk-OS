...
function buildPs1(
  ...
  const lines: string[] = [
    ...
    "",
    "# Digest Auth via CredentialCache (.NET) — compatible PS5.1 y PS7",
    '$netCred = [System.Net.NetworkCredential]::new($USERNAME, $PASSWORD)',
    '$credCache = [System.Net.CredentialCache]::new()',
    '$baseUri = [Uri]::new("http://$DVR_IP`:$DVR_PORT")',
    '$credCache.Add($baseUri, "Digest", $netCred)',
    '$credCache.Add($baseUri, "Basic",  $netCred)',
    "",
    "$all = @()",
    "",
    ...
    "    $resp = [PSCustomObject]@{ Content = $content }",
